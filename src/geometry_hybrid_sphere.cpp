...
void  Geometry_Hybrid_Sphere::compute_forces(double * k_i, const double * Qij)  const 
{
    #pragma omp for simd schedule(dynamic,1) collapse(2)  
  for( int k= 0; k< Nz; k++)
  {
...
void  Geometry_Hybrid_Sphere::Energy_calc(double * k_i,
			       const double * Qij)  const 
{
    ...
    for( int j= 0; j< Ny; j++)
    {
      for( int i= 0; i< Nx; i++)
	      ...
	      if(point_type[(k*Ny+j)*Nx+i] == 1)
        {
          ...
          k_i[(Nx*(Ny*k+j)+i)]=bulk_energy.energy_calculation(QN,dQ,v); 
        }  
	      else if( point_type[(k*Ny+j)*Nx+i] == 2 )
        {
          //check_surface_limits( i,  j,  k);
