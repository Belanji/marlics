...
NoiseDP5::NoiseDP5( GEOMETRY  * lc_pointer, const struct Simulation_Parameters *sim_param ) : Integrator( lc_pointer),
                                   Atol(sim_param->Atol),
                                   Rtol(sim_param->Rtol),
...
void NoiseDP5::evolve( double * Qij, double *time, double tf )
{
              ...
              for( i= 0; i< Nx; i++)
                {
                  ...
                  if(sample_geometry->point_type[(k*Ny+j)*Nx+i] ==1 )
                    {
                      gsl_ran_dir_3d(w, &n[0], &n[1], &n[2]);
                      Qij[5*(Nx*(Ny*k+j)+i)+0]+=(noise_factor*(3.0*n[0]*n[0]-1.0));
                      Qij[5*(Nx*(Ny*k+j)+i)+1]+=(noise_factor*(3.0*n[0]*n[1]));
                      Qij[5*(Nx*(Ny*k+j)+i)+2]+=(noise_factor*(3.0*n[0]*n[2]));
                      Qij[5*(Nx*(Ny*k+j)+i)+3]+=(noise_factor*(3.0*n[1]*n[1]-1.0));
                      Qij[5*(Nx*(Ny*k+j)+i)+4]+=(noise_factor*(3.0*n[1]*n[2]));    
                    }
