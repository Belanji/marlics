...
\replaced[id=e]
{ assuming that you have some version (or a link) of the marlics in one binary folder, you can use the command:}
...
\added[id=e]{to launch a instance of marlics using the ``input\_file.txt'' as input file.}
...
\replaced[id=e]{
 need to include a line with the parameter name and the parameter value separated by space:
}{
type de parameter name followed by any number of spaces and the parameter value afterwards:
}
\begin{lstlisting}
...
\added[id=e]{Be carefull since marlics can accept multiple instances of the same parameter, but will consider just the last one.}
...
\replaced[id=e]{
Finally, in the sphere geometry the liquid crystal is placed inside an ellipsoid with radius equals half of the Grid size i.e. $Nx/2, Ny/2$ and $Nz/2$. Here, one boundary condition has to be defined as the ellipsoid outer layer, giving no space to an periodic boundary condition. Take note that, since we are using a regular grid to define a curvilinear geometry, the geometry resolution greatly depends on the Grid Size.
}{
